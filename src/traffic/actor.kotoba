(ns traffic.actor
  "TrafficActor — the ISCO-08 2164 town and traffic planner actor as
  a `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one planning operation request
  ...
  The unconditional invariant: the TrafficAdvisor can never
  directly issue a binding zoning change, set traffic regulations, or approve
  a development the TrafficGovernor refuses — every
  commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [traffic.advisor :as advisor]
            [traffic.governor :as governor]
            [traffic.store :as store]))

(defn build-graph
  "Build a compiled TrafficActor graph. `store` implements
  `traffic.store/Store`. `advisor` implements
  `traffic.advisor/Advisor` (defaults to `mock-advisor`).
...
(defn run-request!
  "Run one planning operation request to completion or interrupt. `thread-id`
