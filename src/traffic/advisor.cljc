(ns traffic.advisor
  "TrafficAdvisor — proposes a planning operation (draft zoning scenario,
  analyze traffic data, flag public safety concern, schedule public consultation)
  for a registered site. The advisor is swappable: `mock-advisor`
  ...
  it never writes to the store, never issues a binding zoning change or traffic
  regulation, and has no notion of site provenance or planning authority;
  `traffic.governor` is the independent system that decides whether the
  proposal may proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :draft-planning-scenario|:analyze-traffic-data|:flag-public-safety-concern|:schedule-public-consultation
     :effect :propose        ; the advisor NEVER issues binding zoning changes or regulations
...
(def ^:private system-prompt
  "You are a town and traffic planning advisor. Given a planning operation
   request, propose an :op, an honest :confidence (0.0-1.0), and a
   :stake (:low/:medium/:high). Never fabricate confidence you don't
   have. You never issue binding zoning changes or traffic regulations —
   those remain the human planner's/planning authority's exclusive responsibility.")
  ...
  (reify Advisor
    (-advise [_ _store request]
      (let [msgs [{:role :system :content system-prompt}
                  {:role :user :content (str "planning operation request: " (pr-str request))}]
